...
grapmuestra<-function(n,h,t){
  ...
  plot(x,pos1,type="l",col=colores[1],lwd = 2,main=titulo,xlab="Valores del parámetro p",ylab=t,xlim=c(0.05,0.95), ylim=c(min(c(min(pos1),min(pos2),min(pos3),min(pos4),min(pos5)))-0.01, max(c(max(pos1),max(pos2),max(pos3),max(pos4),max(pos5)))+0.01))
